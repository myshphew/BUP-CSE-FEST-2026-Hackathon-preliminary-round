...
3. On a Docker-enabled host, build the image, run the offline container samples, start it with runtime secrets, and run full HTTP samples. The Windows development machine did not have Docker, so these checks are not yet complete.
...
| 0:25–0:55 | The pipeline diagram; GPT-6 Astra turns notes into six supported directive types; code validates before scheduling |
