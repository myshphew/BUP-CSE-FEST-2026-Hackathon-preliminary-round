...
Local Docker verification is complete. The image has not been published, and there is no public API URL yet. The local API is `http://127.0.0.1:8000`, using image `bup-cse-preliminary-round:1.0.0`, Terra/none, and cache 128. Evidence: [docker-verification.json](docker-verification.json).
...
4. Copy the actual image reference/digest from the successful job summary. For this repository, the planned target is `ghcr.io/myshphew/bup-cse-preliminary-round:1.0.0`. **It has not been published yet.**
...
A hosting provider/account and any applicable billing approval are still needed. Use a Docker-capable web service that stays reachable during judging and supports outbound HTTPS to OpenAI. The app binds to `0.0.0.0` and honors platform-provided `PORT`; the default is 8000. No database or persistent volume is required.
...
```powershell
curl.exe https://ACTUAL-HOST/health
.\.venv\Scripts\python.exe -m scripts.evaluate_samples --url https://ACTUAL-HOST --repeat 3 --output output/public-live-benchmark.json
```
